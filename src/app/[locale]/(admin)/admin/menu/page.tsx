...
const FOOD_TAB_LABEL: Record<FoodTab, string> = {
      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <p className="mb-1.5 text-xs font-semibold text-slate-600">Pestaña del menú</p>
              <div className="flex flex-wrap gap-2">
                {FOOD_TABS.map((tab) => (
                  <button
                    key={tab.key}
                    type="button"
                    onClick={() => setForm({ ...form, foodTab: tab.key })}
                    className={`rounded-xl border px-3 py-2 text-sm font-semibold transition-colors ${
                      form.foodTab === tab.key
                        ? "border-cyan-500 bg-cyan-500 text-white"
                        : "border-slate-200 bg-slate-50 text-slate-600 hover:border-cyan-300"
                    }`}
                  >
                    {FOOD_TAB_LABEL[tab.key]}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setForm({
                      ...form,
                      fixedMealPeriod: form.fixedMealPeriod ?? MEAL_PERIODS[0].key,
                      foodTab: "CASADOS",
                    })
                  }
                  className={`rounded-xl border px-3 py-2 text-sm font-semibold transition-colors ${
